...
let componentNames = filenames
    .map(name => name.replace('../svelte/components/', ''))
    .map(name => name.replace('.svelte', ''))

// Create a map of component names to their modules
window.Components = Object.assign({}, ...modules.map((m, index) => ({[componentNames[index]]: m.default})))
